...
class Unit3D:
  ...
  def _build(self, inputs, is_training):
    ...
    """
    net = keras.layers.Conv3D(filters=self._output_channels,
                     kernel_size=self._kernel_shape,
                     strides=self._stride,
                     padding="SAME",
                     use_bias=self._use_bias,
                     name=self._name)(inputs)
    if self._use_batch_norm:
    	with tf.variable_scope(self._name):
	      net = tf.layers.batch_normalization(net, training=is_training, name="batch_norm", scale=False)
...
class InceptionI3d:
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    net = Unit3D(output_channels=64, kernel_shape=[7, 7, 7],
                 stride=[2, 2, 2], name=end_point)._build(net, is_training=is_training)
    ...
    with tf.variable_scope(end_point):
      ...
      with tf.variable_scope('Branch_3'):
        ...
        branch_3 = Unit3D(output_channels=32, kernel_shape=[1, 1, 1],
                          name='Conv3d_0b_1x1')._build(branch_3,
                                                is_training=is_training)
    ...
    with tf.variable_scope(end_point):
      ...
      with tf.variable_scope('Branch_2'):
        branch_2 = Unit3D(output_channels=32, kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')._build(net, is_training=is_training)
        branch_2 = Unit3D(output_channels=128, kernel_shape=[3, 3, 3],
                          name='Conv3d_0a_3x3')._build(branch_2,
                                                is_training=is_training)
      with tf.variable_scope('Branch_3'):
        branch_3 = tf.nn.max_pool3d(net, ksize=[1, 3, 3, 3, 1],
                                    strides=[1, 1, 1, 1, 1], padding="SAME",
    ...
    with tf.variable_scope(end_point):
      ...
      logits = Unit3D(output_channels=self._num_classes,
                      ...
                      use_bias=True,
                      name='Conv3d_0c_1x1')._build(net, is_training=is_training)
